  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 503)
        throw new Error(
          "Model yükleniyor, 20-30 saniye bekleyip tekrar deneyin.",
        );
      if (response.status === 401)
        throw new Error("Geçersiz token. Token'ı kontrol edin.");
      if (response.status === 429)
        throw new Error("Rate limit aşıldı. Birkaç dakika bekleyin.");
